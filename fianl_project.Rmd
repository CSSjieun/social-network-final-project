...
From the plot and the result table, we notice that the distribution is same for both randomly selected and targeted nodes. We assume that this happens because when we only select 1% of the population, the centrality proxy do not affect the infection distribution a lot. Next, we will investigate how the vaccination impact would be performed for the 5% population scenario. 
...
As we can see on the above table, in part c, there is no difference of infection distribution between randomly selected nodes and the targeted nodes. However, in part d, the random selection was higher with 4768 users influenced compared to 4422 for targeted selection, a decrease of 346 users. The difference between part c and part d indicates when there is more population, then the targeted nodes (people) have more power in the trust spreading situation. 
