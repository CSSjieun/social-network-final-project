...
# Step 3: Simulate the SIR model
gp2 <- delete_vertices(g, seed_nodes)
remaining_seeds <- sample(1:vcount(gp2), vcount(gp2) * 0.01)
realization_targeted <- sim_sir(gp2, beta = 0.5, mu = 0.1, seeds = remaining_seeds) %>%
  group_by(t) %>%
  summarize(ninf = n())
...
# Find the smallest time to infection nodes
smallest_time_nodes <- table[order(predictions), ][1:seeds, ]

smallest_time_nodes <- smallest_time_nodes |> drop_na()

cor(smallest_time_nodes$t,smallest_time_nodes[,-c(1,2)])

# Extract node IDs from smallest_time_nodes
seed_nodes <- smallest_time_nodes$inf

# Convert node IDs to indices
seed_indices <- match(seed_nodes, V(g))

# Pass seed indices to sim_sir function
realization_random <- sim_sir(g, beta = 0.5, mu = 0.1, seeds = seed_indices) %>%
  group_by(t) %>% summarize(ninf = n())
```
...
```{r}
set.seed(12345)
seeds <- sample(1:vcount(g),vcount(g)*0.05)
...
seeds <- closeness(g) %>% enframe(name = "inf",value="closeness") %>%
  slice_max(closeness,n=round(vcount(g)*0.05,0))
gp <- delete_vertices(g,seeds_inf)
realization_targeted <- sim_sir(gp,beta = 0.5,mu=0.1,seeds_inf) %>%
  group_by(t) %>% summarize(ninf=n())
