...
The SIR model distribution was plotted with the random and targeted nodes using the results of the random forest model that predicts time of infection of a node using different centrality measures. As on part b), we've chosen the 'closeness' centrality that has the highest correlation with the time.

From the plot, we notice that targeted users which have higher closeness centrality value get trust propagation faster than random nodes, suggesting that targeting the nodes can lead to a more fast and intense spread of trust, which can also be controlled and reduced more quickly. On the other hand, random trust propagation spreads more slowly and has a more prolonged impact.
