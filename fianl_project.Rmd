...
We have plotted the SIR model distribution with the random and targeted nodes. The parameters are β and µ. β is 0.5 (infection rate is 50%) and µ is 0.1 (recovery rate is 10%). From the plot, we are able to notice that targeted nodes which has higher centrality get infected more and bit earlier than the randomly chosen nodes' infection distribution. 
...
Here, we have number of the infected people from random and targeted nodes. Since our data is about the trust, we can interpret that when the nodes are selected randomly, then, the trust spread to 11718 people. If the nodes are selected having high centrality, the, the trust spread to 13117 people. The difference between two distribution is 1459. We can conclude that when nodes have high centrality, trust spreading amount and speed is higher than randomly selected nodes. 
...
Now, we will do the experiment to investigate the impact of controlled situation. We will control some nodes not to spread the trust as much as in the normal situation likewise people get vaccinated. 
...
This is the plot to show the impact of the 'vaccination' to control people not to spread the trust as much as in the normal environment. As we can expect, when people are not controlled, then the probability of trust spread is the highest. However, when people get 'vaccinated' then the probability of trust spreading get reduced. Moreover, when targeted nodes which has high centrality get vaccinated, then the probability even lowered than node from random selection. Therefore, we can conclude that we can control the spreading among people efficiently when we control the highly connected node first. 
...
This is the number of infected people. When there is no vaccination control then the infected people are the highest (11986). But once the spreading is controlled, the number get lowered as we explained above as 10502 for random selection and 7494 for targeted selection. 
...
Even though, we have used the 5-fold cross validation and ridge regularization (L2), the RMSE is still high as 0.87 for the test data set. We consider this happens because the scale of the input data are different so we rather chose random forest which does not require the data scaling. 
