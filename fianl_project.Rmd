...
From the plot and the result table, we notice that the distribution is 4832 for the randomly selected nodes and 4819 for the targeted nodes. The difference between two trust spread users is about 13. Even though the difference is subtle, we can identify that there are more users from the randomly selected nodes from the 1% trust spreading comparison. We can conclude that when there are not too many nodes (people) in the network, then the centrality proxy do not affect a lot for the spreading distribution.  
...
As we can see on the above table, in part c, there are 4832 users from randomly selected nodes network and 4819 users from targeted nodes network. The difference between two figures is 13. This shows when there is only 1% of the people, then trust spreading is more higher for the randomly selected nodes network than the targeted nodes. However, in part d, the random selection was higher with 4768 users who got spread of trust compared to 4422 for targeted selection, a decrease of 346 users. The difference between part c and part d indicates when there is more population, then the targeted nodes (people) have more power in the trust spreading situation. 
